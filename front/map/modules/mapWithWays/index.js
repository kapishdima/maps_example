...
const fetchWays = async (countryId, filters) => {
    const url = `ways?country_id=${countryId}&${createUrlWithFilters(filters)}`;
...
const getWays = async (countryId, options, waysStore) => {
    const ways = await fetchWays(countryId, options);
    waysStore.setWays(ways);
};

const renderWays = (ways, filtersStore, locationsStore) => {
    const onSelectWay = (way) => {
        if (filtersStore.getFilters().way_id === way.id) {
            return;
        }
...
export const createMapWithWays = (mapEl, map) => {
    ...
    const filtersStore = FiltersStore(() => {
        ...
        getWays(countryId, filtersStore.getFilters(), waysStore);
    });
